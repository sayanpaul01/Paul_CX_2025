...
## **📌 Fit Corrmotif Model (K=3) (0.1 Micromolar)**
```{r Model1, echo=TRUE, message=FALSE}
set.seed(11111)
motif.fitted_0.1 <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=3, max.iter=1000, BIC=TRUE, runtype="logCPM")

# View BIC scores
motif.fitted_0.1$bic

# Plot Information Criteria and Motif Patterns
plotIC(motif.fitted_0.1)
plotMotif(motif.fitted_0.1)
```

## **📌 Extract Gene Probabilities (0.1 Micromolar)**
```{r Prob, echo=TRUE, message=FALSE}
# Extract posterior probabilities for genes
gene_prob_tran_0.1 <- motif.fitted_0.1$bestmotif$p.post
...
## **📌 Fit Limma Model Functions**
```{r fit_limma_model_functions1, echo=TRUE, results='hide',message=FALSE, warning=FALSE}
## Fit limma model using code as it is found in the original cormotif code. It has
## only been modified to add names to the matrix of t values, as well as the
## limma fits
...
## **📌 Load Required Libraries**
```{r load_libraries1, echo=TRUE, results='hide',message=FALSE, warning=FALSE}
library(Cormotif)
library(Rfast)
library(dplyr)
library(BiocParallel)
...
## **📌 Fit Corrmotif Model (K=5) (0.5 Micromolar)**
```{r Model4, echo=TRUE, message=FALSE}
set.seed(11111)
motif.fitted<- cormotiffit(exprs=exprs.corrmotif,
                           groupid=groupid_df, compid=compid_df,
                           K=5, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
```{r Prob1, echo=TRUE, message=FALSE}
# Extract posterior probabilities for genes
gene_prob_tran_0.5 <- motif.fitted$bestmotif$p.post
...
prob_2_0.5  <- rownames(gene_prob_tran_0.5[(gene_prob_tran_0.5[,1] >= 0.1 &
                                                  gene_prob_tran_0.5[,2] > 0.5 &
                                                  gene_prob_tran_0.5[,3] > 0.5 &
                                                  gene_prob_tran_0.5[,4] >= 0.02 & # Further relaxed V4
                                                  gene_prob_tran_0.5[,5] < 0.5 & # Allowed more presence in V5
                                                  gene_prob_tran_0.5[,6] < 0.5),]) # Allowed more presence in V6

length(prob_2_0.5)

prob_3_0.5  <- rownames(gene_prob_tran_0.5[(gene_prob_tran_0.5[,1] <0.5 & gene_prob_tran_0.5[,2] <0.5 & gene_prob_tran_0.5[,3] <0.5 & gene_prob_tran_0.5[,4] >0.5 & gene_prob_tran_0.5[,5] > 0.5 & gene_prob_tran_0.5[,6]>=0.02),])
length(prob_3_0.5)

prob_4_0.5  <- rownames(gene_prob_tran_0.5[(gene_prob_tran_0.5[,1] <0.5 & gene_prob_tran_0.5[,2] <0.5 & gene_prob_tran_0.5[,3] <0.5 & gene_prob_tran_0.5[,4] <0.5 & gene_prob_tran_0.5[,5] > 0.5 & gene_prob_tran_0.5[,6]>0.5),])
length(prob_4_0.5)
...
```{r box2, echo=TRUE, results='hide',message=FALSE, warning=FALSE}
