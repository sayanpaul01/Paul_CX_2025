...
go_matrix_df <- purrr::map_dfr(names(go_files), function(cond) {
  ...
  if (nrow(df) == 0 || !all(c("ID", "Description", "pvalue") %in% colnames(df))) {
    message("⚠️ Skipping or padding malformed file: ", cond)
    return(tibble(Description = rbio_descriptions, pvalue = NA, log10p = NA, Condition = cond))
  }
...
go_matrix_df <- purrr::map_dfr(names(go_files), function(cond) {
  ...
  if (nrow(df) == 0 || !all(c("ID", "Description", "pvalue") %in% colnames(df))) {
    message("⚠️ Skipping or padding malformed file: ", cond)
    return(tibble(Description = g4_descriptions, pvalue = NA, log10p = NA, Condition = cond))
  }
  ...
  results <- lapply(names(g4_terms), function(go_id) {
    ...
    if (nrow(df_sub) == 0) {
      tibble(Description = g4_terms[[go_id]], pvalue = NA, log10p = NA, Condition = cond)
    } else {
      best_row <- df_sub %>% dplyr::slice_min(pvalue, n = 1)
      tibble(
...
go_matrix_df <- purrr::map_dfr(names(go_files), function(cond) {
  ...
  if (nrow(df) == 0 || !all(c("ID", "Description", "pvalue") %in% colnames(df))) {
    message("⚠️ Skipping or padding malformed file: ", cond)
    return(tibble(Description = rnapolI_descriptions, pvalue = NA, log10p = NA, Condition = cond))
  }
  ...
  results <- lapply(names(parent_terms), function(go_id) {
    df_sub <- df %>% dplyr::filter(ID == go_id)
    if (nrow(df_sub) == 0) {
      tibble(Description = parent_terms[[go_id]], pvalue = NA, log10p = NA, Condition = cond)
    } else {
      best_row <- df_sub %>% dplyr::slice_min(pvalue, n = 1)
      tibble(
