...
# Define Entrez IDs of interest (DDR genes)
entrez_ids <- c(
...
# Define category mapping for Entrez IDs (based on final user-provided classification)
entrez_category <- tribble(
  ~ENTREZID, ~Category,
  5366, "Apoptosis", 55367, "Apoptosis", 8795, "Apoptosis", 355, "Apoptosis",
  27113, "Apoptosis", 317, "Apoptosis", 836, "Apoptosis", 841, "Apoptosis",
  842, "Apoptosis", 581, "Apoptosis", 637, "Apoptosis", 54205, "Apoptosis",
  ...
  25, "Miscellaneous / Broad", 207, "Miscellaneous / Broad"
)
...
# Merge with Entrez category
all_data <- left_join(all_data, entrez_category, by = c("Entrez_ID" = "ENTREZID"))
...
# Reorder genes alphabetically by category
...
category_colors <- structure(
  c("darkorange", "steelblue", "darkgreen", "firebrick", "gold", "gray60"),
  names = sort(unique(gene_order_df$Category))
)
...
# Read TOP2A target Entrez IDs from CSV
...
# Load DEG data and tag with condition + drug
top2a_data_raw <- map2_dfr(deg_files, names(deg_files), function(file, label) {
  read_csv(file, show_col_types = FALSE) %>%
...
complete_top2a <- all_combos %>%
  left_join(top2a_data_raw, by = c("Entrez_ID", "Condition", "Drug")) %>%
  mutate(
    logFC = ifelse(is.na(logFC), 0, logFC),
    adj.P.Val = ifelse(is.na(adj.P.Val), 1, adj.P.Val)
  ) %>%
...
# Ensure Drug factor order for color consistency
complete_top2a$Drug <- factor(complete_top2a$Drug, levels = c("CX.5461", "DOX"))

# Plot logFC boxplot (default y-scale)
...
# Read TOP2A literature-based target Entrez IDs from CSV
...
# Ensure Drug factor order for color consistency
complete_top2a_lit$Drug <- factor(complete_top2a_lit$Drug, levels = c("CX.5461", "DOX"))
