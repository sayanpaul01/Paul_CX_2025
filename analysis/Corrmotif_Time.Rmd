...
# Plot Information Criteria (IC)
plotIC(motif.fitted)
```


## **📌 Fit Corrmotif Model (K=2) (CX 0.1 (3hr, 24hr and 48hr))**
```{r Model1, echo=TRUE, message=FALSE}
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=2, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
# Plot Motif Pattern (X-axis = Timepoints: 3h, 24h, 48h)
...
gene_prob_CX_0.1 <- motif.fitted$bestmotif$p.post
...
## Fit Corrmotif Model (K=1:8) (CX 0.5 (3hr, 24hr and 48hr))
```{r Model2, echo=TRUE, message=FALSE}
set.seed(11111)
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=1:8, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
## **📌 Fit Corrmotif Model (K=2) (CX 0.5 (3hr, 24hr and 48hr))**
```{r Model3, echo=TRUE, message=FALSE}
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=2, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
# Extract posterior probabilities for genes
gene_prob_CX_0.5 <- motif.fitted$bestmotif$p.post
...
## Load Corrmotif Data
```{r load_data4, echo=TRUE, results='hide', message=FALSE}
# Read the Corrmotif Results
Corrmotif <- read.csv("data/Corrmotif/CX5461.csv")
Corrmotif_df <- data.frame(Corrmotif)
rownames(Corrmotif_df) <- Corrmotif_df$Gene
...
## Fit Corrmotif Model (K=1:8) (DOX 0.1 (3hr, 24hr and 48hr))
```{r Model5, echo=TRUE, message=FALSE}
set.seed(11111)
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=1:8, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
## **📌 Fit Corrmotif Model (K=2) (DOX 0.1 (3hr, 24hr and 48hr))**
```{r Model6, echo=TRUE, message=FALSE}
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=2, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
# Extract posterior probabilities for genes
gene_prob_DOX_0.1 <- motif.fitted$bestmotif$p.post
...
## Fit Corrmotif Model (K=1:8) (DOX 0.5 (3hr, 24hr and 48hr))
```{r Model7, echo=TRUE, message=FALSE}
set.seed(11111)
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=1:8, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
## **📌 Fit Corrmotif Model (K=2) (DOX 0.5 (3hr, 24hr and 48hr))**
```{r Model8, echo=TRUE, message=FALSE}
motif.fitted <- cormotiffit(exprs=exprs.corrmotif,
                            groupid=groupid_df, compid=compid_df,
                            K=2, max.iter=1000, BIC=TRUE, runtype="logCPM")
...
```{r Prob5, echo=TRUE, message=FALSE}
# Extract posterior probabilities for genes
gene_prob_DOX_0.5 <- motif.fitted$bestmotif$p.post
