...
all_toptables_0.1 <- bind_rows(
  Toptable_CX_0.1_3_df  %>% mutate(Drug = "CX.5461", Timepoint = "3"),
  Toptable_CX_0.1_24_df %>% mutate(Drug = "CX.5461", Timepoint = "24"),
  Toptable_CX_0.1_48_df %>% mutate(Drug = "CX.5461", Timepoint = "48"),
  Toptable_DOX_0.1_3_df  %>% mutate(Drug = "DOX", Timepoint = "3"),
  Toptable_DOX_0.1_24_df %>% mutate(Drug = "DOX", Timepoint = "24"),
  Toptable_DOX_0.1_48_df %>% mutate(Drug = "DOX", Timepoint = "48")
)
...
all_toptables_0.1 <- all_toptables_0.1 %>% filter(!is.na(Response_Group))
...
ggplot(data_summary, aes(x = Timepoint, y = mean_abs_logFC, group = Drug, color = Drug)) +
  ...
  facet_wrap(~ Response_Group, ncol = 1) +  # Facet by Response Group
  theme_bw() +
  labs(
    x = "Timepoints",
...
all_toptables_0.1$Timepoint <- factor(all_toptables_0.1$Timepoint, 
                                      levels = c("Timepoint: 3 hours", "Timepoint: 24 hours", "Timepoint: 48 hours"))
...
# Convert factors to ensure correct ordering
data_summary$Timepoint <- factor(data_summary$Timepoint, 
                                 levels = c("Timepoint: 3 hours", "Timepoint: 24 hours", "Timepoint: 48 hours"))
...
ggplot(data_summary, aes(x = Timepoint, y = mean_logFC, group = Drug, color = Drug)) +
  ...
  ylim(-2, 1.5) +
  facet_wrap(~ Response_Group, ncol = 1) +
  theme_bw() +
  labs(x = "Timepoints", y = "Mean Log Fold Change", title = "Mean Log Fold Change Across Response Groups (0.1 micromolar)") +
  theme(
    plot.title = element_text(size = rel(1.5), hjust = 0.5),
    strip.text = element_text(size = 12, face = "bold")
  )
...
# Data
data <- data.frame(
  Category = c("Non response (0.5)", "DOX-only early-mid response (0.5 µM)", "DOX only mid-late response (0.5 µM)", "CX-only mid-late response (0.5 µM)", "CX-DOX mid-late response (0.5 µM)"),
...
all_toptables_0.5 <- all_toptables_0.5 %>%
  mutate(
    Response_Group = case_when(
      ...
      Entrez_ID %in% prob_2_0.5 ~ "DOX-only early-mid response\n(0.5 micromolar)",
      Entrez_ID %in% prob_3_0.5 ~ "DOX only mid-late response\n(0.5 micromolar)",
      Entrez_ID %in% prob_4_0.5 ~ "CX-only mid-late response\n(0.5 micromolar)",
      Entrez_ID %in% prob_5_0.5 ~ "CX-DOX mid-late response\n(0.5 micromolar)",
      TRUE ~ NA_character_
    )
  )
...
all_toptables_0.5 <- all_toptables_0.5 %>%
  mutate(
    ...
    Response_Group = factor(Response_Group,
                            levels = c("Non response\n(0.5 micromolar)",
                                       "DOX-only early-mid response\n(0.5 micromolar)",
                                       "DOX only mid-late response\n(0.5 micromolar)",
                                       "CX-only mid-late response\n(0.5 micromolar)",
                                       "CX-DOX mid-late response\n(0.5 micromolar)"))
  )
...
# Convert factors to ensure correct ordering
all_toptables_0.5$Timepoint <- factor(all_toptables_0.5$Timepoint, 
                                      levels = c("Timepoint: 3 hours", "Timepoint: 24 hours", "Timepoint: 48 hours"))
